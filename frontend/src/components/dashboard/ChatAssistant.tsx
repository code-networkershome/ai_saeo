...
export const ChatAssistant: React.FC = () => {
    ...
    return (
        <div className="fixed inset-y-0 right-0 z-50 flex items-center pointer-events-none">
            {/* Toggle Button (Floating Left of Sidebar) */}
            <div className="pointer-events-auto">
                <button
                    onClick={() => setIsOpen(!isOpen)}
                    className={`mr-4 p-4 rounded-2xl shadow-2xl transition-all duration-300 group flex items-center gap-3 ${isOpen ? 'bg-slate-800 -translate-x-4' : 'bg-gradient-to-br from-indigo-600 to-purple-600 hover:scale-105'
                        }`}
                    title={isOpen ? "Minimize Assistant" : "Need help? Ask our Co-Pilot"}
                >
                    {isOpen ? (
                        ...
                        <>
                            <div className="relative">
                                ...
                                <div className="absolute -top-1 -right-1 w-3 h-3 bg-emerald-500 border-2 border-indigo-600 rounded-full" />
                            </div>
                            <span className="text-xs font-bold text-white pr-1">Co-Pilot</span>
                        </>
                    )}
                </button>
            </div>
            ...
            <AnimatePresence>
                {isOpen && (
                    ...
                    >
                        {/* Header */}
                        <div className="p-6 bg-gradient-to-br from-indigo-600/20 via-transparent to-purple-600/10 border-b border-white/10">
